...
(define-type Value
  ...
  [symV (sym : symbol)]
  [nullV ]
  [regularMethodV (name : symbol) (arg : symbol) (body : ExprC)]
  [primitiveMethodV (name : symbol) (primNumber : number)]
...
(define ClasseObject
        (classV 'null 'dummy (regularMethodV 'met1 'none (numC 42))
                             (regularMethodV 'met2 'none (numC 42))))
...
(define (lookup-method [name : symbol] [obj : Value]) : Value
      (cond
          [(equal? obj ObjetoObject) (error 'lookup-method "chamar metodo mensagem desconhecida")]
          [else (cond
              [(symbol=? name (regularMethodV-name (classV-met1 (objectV-classe obj))))
                              (classV-met1 (objectV-classe obj))]
              [(symbol=? name (regularMethodV-name (classV-met2 (objectV-classe obj))))
...
(define (interp [a : ExprC] [objectEnv : Env]) : Value
  (type-case ExprC a
    ...
    [regularMethodC (n a b) (regularMethodV n a b)]

    [primitiveMethodC (n p) (primitiveMethodV n p)]
    ...
    [sendC (o m a)
        (let* ([objeto (interp o objectEnv)]
              ...
              (interp (regularMethodV-body method)
                 (extend-env (bind (regularMethodV-arg method) (box (interp a objectEnv))) objEnv)))]
